...
#
#   bash tools/deb/package-gate.sh                   every pool, static checks and the rebuild
#   bash tools/deb/package-gate.sh --arch <arch>     one pool (a native per-architecture CI job)
#   bash tools/deb/package-gate.sh --static          every pool, no rebuild (the CI job that merges the pools)
#   bash tools/deb/package-gate.sh --board <board> [--arch <arch> | --static]
#                                                    that board's pool only: the producers of the packages
#                                                    boards/boards.tsv lists for it, at its architecture
#
#   reads   _out/debs/<arch>/pool/*.deb          (built by `make pool`)
#   asserts the facts listed below, per architecture
#
# Facts are read out of the archives with dpkg-deb (inside the mica-build-env base image);
...
while [ "$#" -gt 0 ]; do
    case "$1" in
    --arch) case "${2:-}" in amd64 | arm64) ONLY_ARCH="$2"; shift 2 ;; *) echo "error: --arch takes amd64 or arm64" >&2; exit 1 ;; esac ;;
    --static) STATIC_ONLY=1; shift ;;
    --board) [ -n "${2:-}" ] || { echo "error: --board takes a board name" >&2; exit 1; }; BOARD="$2"; shift 2 ;;
    *) echo "usage: bash tools/deb/package-gate.sh [--board <board>] [--arch <amd64|arm64> | --static]    (it reads the pools under _out/debs)" >&2; exit 1 ;;
    esac
done
...
command -v docker >/dev/null 2>&1 || {
    echo "error: docker is required and not on PATH. dpkg-deb runs inside the mica-build-env base image rather than on the host, and the reproducibility check drives a real package build" >&2
    exit 1
}

# The producer set, captured rather than piped so a discovery refusal is not swallowed;
# for one board, the producers of its packages.
if [ -n "${BOARD}" ]; then
...
# so alone it means both) -- read from those declarations, never discovered
# from _out/debs, so a missing pool fails.
declared_arches=" "
...
[ "${#ARCHES[@]}" -gt 0 ] || { echo "error: no producer names an architecture, so there is no pool to gate" >&2; exit 1; }
# A board's pool is its architecture's alone.
[ -z "${BOARD}" ] || ARCHES=("${BOARD_ARCH}")
if [ -n "${ONLY_ARCH}" ]; then
    case " ${ARCHES[*]} " in *" ${ONLY_ARCH} "*) ARCHES=("${ONLY_ARCH}") ;; *) echo "error: no producer builds for ${ONLY_ARCH}" >&2; exit 1 ;; esac
fi
for arch in "${ARCHES[@]}"; do
    [ -d "${DIST}/${arch}/pool" ] || {
        echo "error: ${DIST}/${arch}/pool does not exist, so there is nothing to check for ${arch}. Build it with \`make pool\`; a gate that skipped the missing architecture would report on half a pool" >&2
        exit 1
    }
done
...
*)
    echo "error: $(uname -m) is not an architecture the build-env images carry, so there is no container to read the archives in" >&2
    exit 1
    ;;
esac
...
for arch in "${ARCHES[@]}"; do
    ...
    if [ "${got_set}" = "${EXPECTED_SET}" ]; then
        ...
        fail "${arch}: the pool holds [${got_set% }], but the producers building for ${arch} declare and the lock imports [${EXPECTED_SET% }]. A missing package is one the composer cannot install; an extra one is an archive no producer owns and no lock row names. \`make pool\` fetches the imports and builds the rest"
    fi
    ...
    for producer in ${pool_producers}; do
        ...
        if [ "${found_n}" = 0 ]; then
            fail "${arch}: the producer '${producer}' contributed NO archive to ${pool}, though it declares [${want_pkgs# }] and builds for ${arch}. \`make pool\` runs every discovered producer; this one built nothing, or its output went somewhere else"
        elif [ -n "${missing}" ]; then
...
if [ "${STATIC_ONLY}" = 1 ]; then
    echo "note: --static: the reproducibility rebuild runs in each architecture's own job (--arch)"
    echo "RESULT: PASS (${STATIC_PASS}/${STATIC_PASS} checks passed, ${ARCHIVES_N} archives, ${PATHS_N} payload paths, ${SCRIPTS_N} maintainer scripts, ${ALL_COMPARED_N} all-architecture archives compared, ${VIRTUAL_RESOLVED_N} local-virtual dependencies resolved)"
    exit 0
fi
