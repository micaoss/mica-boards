...
for p in "${REPO_ROOT}/Makefile" "${VERSION_FILE}"; do
    [ -e "${p}" ] || {
        echo "error: ${p} does not exist. tools/deb/version.sh derives the repository as two levels above itself and reads its VERSION file; if this file moved, that arithmetic moved with it" >&2
        exit 1
    }
done

[ "$#" -eq 0 ] || {
    echo "usage: bash tools/deb/version.sh    (no arguments; there is one pool and it has one version)" >&2
    exit 1
}
