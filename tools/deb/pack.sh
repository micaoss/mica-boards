...
#
# Runs inside the mica-build-env base image at <arch> at the package's architecture:
# dpkg-shlibdeps resolves against the libraries of the container it runs in.
# tools/deb/README.md is the contract this implements.
#
# mica-build-side: container -- every line below runs in the mica-build-env base image at <arch>,
...
while [ "$#" -gt 0 ]; do
    ...
    *) die "'$1' is not an option this packer takes; see tools/deb/README.md" ;;
    esac
done
...
if [ "${ARCH}" != all ] && [ "${ARCH}" != "${CONTAINER_ARCH}" ]; then
    die "--arch ${ARCH} in a ${CONTAINER_ARCH} container. dpkg-shlibdeps resolves the libraries of the architecture it RUNS on, so this would record ${CONTAINER_ARCH}'s dependency versions in an ${ARCH} package and fail on the device, not here. Run this inside the mica-build-env base image:${ARCH}"
fi
...
for f in Package Version Architecture Maintainer Section Priority Description; do
    [ -n "$(control_field "${f}" "${CONTROL}")" ] ||
        die "--control ${CONTROL} declares no ${f}. Every field of the template is required; tools/deb/README.md lists them and shows one"
done
